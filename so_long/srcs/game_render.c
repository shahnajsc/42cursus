...
static mlx_image_t	*texture_process(t_slong *slong, char *img_path)
{
	...
	{
		printf("texture erro\n");
		game_error(slong, "texture fail"); // change msg
	}
	...
	{
		printf("image erro\n");
		game_error(slong, "image fail"); // change msg
	}
	return(image);
}
...
static void	image_process(t_slong *slong)
{
	...
	if (!slong->image)
		game_error(slong, "image calloc failed"); // change msg
	...
	slong->image->exit = texture_process(slong, EXIT_I);
}

static void	window_img_load(t_slong *slong, mlx_image_t *img, int row, int col)
{
	int win_image;

	win_image = mlx_image_to_window(slong->mlx, img, col * PX, row * PX);
	if (win_image < 0)
		game_error(slong, "win_img error");
}
...
void	game_map_load(t_slong *slong)
{
	...
	while (++row < slong->map->row_c)
	{
		...
		while (++col < slong->map->col_c)
		{
			if (slong->map->map[row][col] == '1')
			...
			else if (slong->map->map[row][col] == '0')
				...
				window_img_load(slong, slong->image->exit, row, col);
