...
/*   Updated: 2024/12/17 15:55:43 by shachowd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static mlx_image_t	*texture_process(t_slong *slong, char *img_path)
{
	...
	if (!texture)
		game_error(slong, "texture loading failed", EXIT_FAILURE);
	image = mlx_texture_to_image(slong->mlx, texture);
	mlx_delete_texture(texture);
	if (!image)
		game_error(slong, "image loading failed", EXIT_FAILURE);
	return (image);
}
...
static void	image_process(t_slong *slong)
{
	...
	if (!slong->image)
		game_error(slong, "image calloc failed", EXIT_FAILURE);
...
void	window_load(t_slong *slong, mlx_image_t *img, int row, int col)
{
	...
	if (win_image < 0)
		game_error(slong, "image loading to window is failed", EXIT_FAILURE);
}
...
void	game_map_load(t_slong *slong)
{
	...
	while (++row < slong->map->row_c)
	{
		...
		{
			window_load(slong, slong->image->floor, row, col);
			if (slong->map->map[row][col] == '1')
				window_load(slong, slong->image->wall, row, col);
			else if (slong->map->map[row][col] == 'C')
				window_load(slong, slong->image->collectible, row, col);
			else if (slong->map->map[row][col] == 'E')
			{
				window_load(slong, slong->image->exit_op, row, col);
				window_load(slong, slong->image->exit_cl, row, col);
			}
			else if (slong->map->map[row][col] == 'P')
				window_load(slong, slong->image->player, row, col);
		}
	}
}
