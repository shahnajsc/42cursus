...
/*   Updated: 2024/12/13 17:08:57 by shachowd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	game_movement(t_slong *slong)
{
	if (slong->map->map[slong->next_r][slong->next_c] == '1')
		return ;
	if (slong->map->map[slong->next_r][slong->next_c] == 'C')
	{
		slong->collec_picked++;
		slong->map->map[slong->next_r][slong->next_c] = '0';
			//if (slong->collec_picked == slong->map->collec_c)
			 // change exit img
	}
	if (slong->map->map[slong->next_r][slong->next_c] == 'E' &&
				(slong->collec_picked == slong->map->collec_c))
	{
			ft_putstr_fd("you won the game", 1);
			game_close(slong, EXIT_SUCCESS);
	}
	window_img_load(slong, slong->image->player, slong->next_r, slong->next_c);
	...
	slong->moves++;
	slong->current_r = slong->next_r;
	slong->current_c = slong->next_c;
...
void	key_hook_handle(mlx_key_data_t keypressed, void *param)
{
	...
	if (slong->map->map[slong->current_r][slong->current_c] != '1')
	{
		if (keypressed.key == MLX_KEY_ESCAPE && keypressed.action == MLX_PRESS)
			...
			slong->next_r = slong->current_r - 1;
			...
			slong->next_c = slong->current_c - 1;
